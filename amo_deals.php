...
require_once '_dataSource.class.php';
	$dataSource = new DataSource('select r.yc_id as recordId, r.yc_client_id as clientId, r.date_last as dateLast, r.stat as stat, r.is_deleted as isDeleted, c.name as name, c.spent as spent, c.amo_id as amoId from records_autobeauty r join clients_autobeauty c on r.yc_client_id = c.yc_id where r.yc_id = 117017553');
...
$data = array();
		...
		$data[0]['name'] = $item['name'] . ' (YCLIENTS)';
		$data[0]['price'] = (int)$item['spent'];
		...
		$data[0]['created_at'] = strtotime($item['dateLast']);
		$data[0]['_embedded'] = array('contacts' => array(array('id' => (int)$item['amoId'])));
